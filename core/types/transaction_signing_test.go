...
func TestEIP155ChainId(t *testing.T) {
	...
	if ChainId(tx).Cmp(signer.chainId) != 0 {
		t.Error("expected chainId to be", signer.chainId, "got", ChainId(tx))
	}
	...
	if ChainId(tx).Sign() != 0 {
		t.Error("expected chain id to be 0 got", ChainId(tx))
	}
}
